...
class ControllerAccountAddress extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['addresses'][] = array(
                'result'     => $result,
                'boxy_id'    => 'colorboxAddrEdit' . $result['address_id'],
                'layer_addr_edit'   => $this->getForm($result['address_id']),
        ...
        $data['link_my_order'] = $this->url->link('account/order', '', 'SSL');

        $data['layer_addr_add'] = $this->getForm(null);
	...
	protected function getForm($address_id) {
		...
		if (!isset($address_id)) {
			...
			$data['action'] = $this->url->link('account/address/edit', 'address_id=' . $address_id, 'SSL');
		}
        ...
        if (!isset($address_id))
        {
            $data['boxy_id'] = "colorboxAddr";
        }
        else
        {
            $data['boxy_id'] = "colorboxAddrEdit" . $address_id;
        }

		$data['back'] = $this->url->link('account/address', '', 'SSL');

        // add popup layer
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/common/addr_layer.tpl')) {
            return $this->load->view($this->config->get('config_template') . '/template/common/addr_layer.tpl', $data);
        }
        return null;
	}
