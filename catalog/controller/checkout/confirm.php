...
class ControllerCheckoutConfirm extends Controller {
	public function index() {
        ...
        // DEBUG: output validate infomation
		...
		if (!$redirect) {
			...
			foreach ($this->cart->getProducts() as $product) {
                ...
                if ($product['image']) {
                    $image = $this->model_tool_image->resize($product['image'], 150, 225);
                } else {
                    $image = '';
                }
			...
			foreach ($order_data['totals'] as $total) {
				$data['totals'][$total['title']] = $this->currency->format($total['value']);
			}
        ...
        $data['link_pay'] = $this->url->link('payment/pp_express/checkout', '', 'SSL');
